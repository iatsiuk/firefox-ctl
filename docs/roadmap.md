...
Five ralphex plans, executed in order. Each plan was complete, tested and linted before the next started. Plans 1 and 2 had no dependency on each other apart from `protocol` types; plans 3-5 built on 2. All five are delivered, plus two interim plans written during execution: command deadlines and the AMO public listing. The plan files themselves are working notes and are not tracked in git.
