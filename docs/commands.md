...
| Command | Params | Notes |
|---|---|---|
| getContent | [selector], [includeHtml], [maxLength=50000] | text is the browser's `innerText` of the root, trimmed at both ends with internal whitespace kept, so script, style and hidden descendant text is absent and `<br>` and block boundaries are newlines; a root with `display: none` on itself or an ancestor, and the page branch of a document without a `body`, answer `""` with `hidden: true` and are never read, every other root answers `hidden: false`; SVG and other non-HTML roots answer raw `textContent`. With a selector `{selector, text, tagName, textLength, truncated, hidden}`, without one `{url, title, text, textLength, truncated, hidden}`; `includeHtml` adds `html` - `innerHTML` of the element, `documentElement.outerHTML` for the page, never truncated; over `maxLength` the text ends in `\n\n[... truncated, use selector for specific content]` |
