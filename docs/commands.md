...
`Hint: Use getPageState to see available elements.` The suggestions that match what the
user reads - buttons, links, fields by label or placeholder, `aria-label` - are
rendered elements only; the near-miss selectors (`id`, class, input `name`,
`data-testid`) may be hidden, because a near miss on a hidden element is still the
selector the caller was looking for. A required non-selector param missing its value
fails the same way: `text is required` for `type`, `key is required` for `pressKey`,
...
| Command | Params | Notes |
|---|---|---|
| getContent | [selector], [includeHtml], [maxLength=50000] \| [tail] | text is the browser's `innerText` of the root, trimmed at both ends with internal whitespace kept: for a root with layout boxes that is the rendered text, script, style and hidden descendant text absent, `<br>` and block boundaries newlines; for a root the browser considers not rendered (the content of a closed `<details>`, a detached element) it falls back to raw `textContent` and so does the result; a `display: contents` root is a spec-level fallback case too, but Firefox renders it like any other root, which is the behaviour observed and relied on; a root with `display: none` on itself or an ancestor, and the page branch of a document without a `body`, answer `""` with `hidden: true` and are never read, every other root answers `hidden: false`; SVG and other non-HTML roots answer raw `textContent`; none of these fallbacks is filtered. With a selector `{selector, text, tagName, textLength, truncated, hidden}`, without one `{url, title, text, textLength, truncated, hidden}`; `includeHtml` adds `html` - `innerHTML` of the element, `documentElement.outerHTML` for the page, never truncated and never cut by either limit; over `maxLength` the text ends in `\n\n[... truncated, use selector for specific content]`. `--tail N` returns the last N characters instead, behind the marker `[... <dropped> characters before this point]` and a blank line, and adds `tailLength`, the kept characters without the marker; `textLength` is always the full length and `truncated` says whether anything was dropped. N must be a positive integer (`tail must be a positive integer`), `tail` and `maxLength` are mutually exclusive by presence whatever the value (`tail and maxLength are mutually exclusive`), so `maxLength` and its default do not apply with `tail`; both limits count UTF-16 code units, and a tail may split a surrogate pair |
| click | selector \| text, [scope], [autoWait=true], [waitTimeout=5000] | polls every 100 ms until `waitTimeout` when `autoWait`, scrolls the element into view, then `element.click()`; `{selector, clicked: true, tagName, text, id, className, matchedBy}` with `id` and `className` null when absent and `matchedBy` naming the parameter that found the element, `"selector"` or `"text"`; `text` is the element's visible text captured before the click, so a handler that hides or removes the element does not change it. With `text` the nearest actionable ancestor-or-self wins, two or more targets answer `AMBIGUOUS_TEXT` without clicking and `selector` is generated for the element, or `null`; see Text targeting above |
