...
- Shared building blocks: `selector.ts` validates a selector and polls for a late element (`smartQuerySelector`, 100 ms between double-`raf` frames until the timeout), `timing.ts` holds the frame and sleep helpers, `suggest.ts` builds the `Element not found` message with alternative selectors and page context
