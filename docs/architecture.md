...
- Injection trigger: `onDOMContentLoaded` for a child frame (`frameId !== 0`) of a watched tab whose document url matches the watch glob records a pending injection and calls `tabs.executeScript(tabId, {frameId, file: "/dist/content.js", runAt: "document_idle", matchAboutBlank: true})`. `matchAboutBlank` covers a zoid-style frame that sits permanently on `about:blank` or `about:srcdoc` (a hidden mediator, a field before its first real navigation): without it Firefox refuses the injection and the frame never joins `listFrames`, even though "every child frame" is observed without `--match`. `contentScripts.register` is not used: it registers by url for the whole browser, not for one tab. There is no backfill of frames already open when the watch opened, so the contract is watch-before-load and an unobserved frame answers `FRAME_NOT_OBSERVED`. There is also no `documentId`: two navigations of the same frame to the identical url within one generation are treated as one event, so a reload to that same url while the first injection is still pending can leave the fresh document uninjected until it navigates elsewhere - a narrow gap that degrades the same clean way as BFCache, `FRAME_NOT_OBSERVED` rather than a stale answer
